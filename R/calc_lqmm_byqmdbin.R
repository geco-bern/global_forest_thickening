calc_lqmm_byqmdbin <- function(df, binwidth = 0.25, breaks = NA){
  ...
  # --- 1. Create QMD breaks at fixed intervals (default 0.25) ---
  if (identical(breaks, NA)) {
  ...
  # --- 3. Nest and fit models including logQMD_sc ---
  df <- df |>
    group_by(bin_lqmm) |>
    nest() |>
...
extract_row <- function(sum, pattern = "year") {
  ...
  if (length(idx) == 0) return(NA_integer_)  # no match
  idx[1]
}
...
determine_failed <- function(out) {
  ...
  if (inherits(out, "try-error")) return(TRUE)
  
  # summary is not a list
  if (!is.list(out)) return(TRUE)
  
  # tTable missing
  if (is.null(out$tTable)) return(TRUE)
  
  # no rows (rare but possible)
  if (nrow(out$tTable) == 0) return(TRUE)
