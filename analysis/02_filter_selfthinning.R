...
filter_stl_slope2 <- function(df){

  df <- data_unm_und_noin_qmdfilt$data[[1]]
  ...
  df_byplot <- df |>
    ...
    mutate(
      ...
      slopes_seg = map(segmod, ~ if (is.null(.x)) NA_real_ else slope(.x)$logQMD_sc[, "Est."])
    )
